...
class DryRunProvider(BaseProvider):
    ...
    def run(self, prompt: str) -> str:
        ...
        is_storyboard = "Storyboard Agent" in prompt
        if is_storyboard:
            return self._create_storyboard_stub(prompt)
    ...
    def _create_storyboard_stub(self, prompt: str = "") -> str:
        """
        Return a deterministic, contract-compliant stub for storyboard_agent.
        Includes quality bar fields.
        """
        target_modules = 6
        import re
        match = re.search(r"MODULE_COUNT_TARGET:\s*(\d+)", prompt)
        ...
        # We also need to see if curriculum modules were provided in the prompt to match dynamic count
        # For a truly robust stub parsing the prompt's JSON state would be best, but targeting 6 or extracting from prompt works for dry runs
        modules_match = re.search(r'"modules": \[.*?\]', prompt, re.DOTALL)
        if modules_match:
            try:
                # very crude heuristic, real provider would just return the expected schema length
                pass
            except:
                pass
